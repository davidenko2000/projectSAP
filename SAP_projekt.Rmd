...
Jako mala p-vrijednost nam ukazuje da postoji statistički značajna razlika u "ladder score-u" u dvije godine.Na razini značajnosti od 5% možemo odbaciti hipotezu $H_0$. Postoje značajne razlike u sreći država u dvije godine tj. države u 2021. godini su sretnije nego u 2020.
\eject
...
Analogno računamo za preostale nezavisne varijable.
...
```{r}
fit_reg = lm(`Ladder score` ~ `Logged GDP per capita` + `Social support` + `Freedom to make life choices`+ `Regional indicator`, data=whr2021)
summary(fit_reg)
```
